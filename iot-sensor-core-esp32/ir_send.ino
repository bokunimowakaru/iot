...
#define IR_DATA_SIZE	16			// データ長(byte),4の倍数、16以上

#define IR_FLASH_AEHA_TIMES		16	// シンボルの搬送波点滅回数（ＡＥＨＡ）
#define IR_FLASH_NEC_TIMES		22	// シンボルの搬送波点滅回数（ＮＥＣ）
#define IR_FLASH_SIRC_TIMES		24	// シンボルの搬送波点滅回数（ＳＩＲＣ）
#define IR_FLASH_ON				11	// LED ON 期間 us (規格上 ON+OFFで 23 us)
#define IR_FLASH_OFF			11	// LED ON 期間 us (規格上 ON+OFFで 23 us)

// enum IR_TYPE{ AEHA=1, NEC=2, SIRC=3 };		// 家製協AEHA、NEC、SONY SIRC切り換え
#define IR_AEHA		1
#define IR_NEC		2
#define IR_SIRC		3
...
void ir_flash(byte times){
	while(times){
		times--;
		delayMicroseconds(IR_FLASH_ON);
		digitalWrite(PIN_IR_OUT, IR_OUT_ON);
		delayMicroseconds(IR_FLASH_OFF);
		digitalWrite(PIN_IR_OUT, IR_OUT_OFF);
	}
}
...
void ir_send(byte *data, const byte data_len, const byte ir_type ){
	...
	switch( ir_type ){
		case IR_NEC:
			ir_flash( 8 * IR_FLASH_NEC_TIMES );	// send 'SYNC_H'
			ir_flash( 8 * IR_FLASH_NEC_TIMES );
			ir_wait(  8 * IR_FLASH_NEC_TIMES );	// send 'SYNC_L'
			for( i = 0 ; i < data_len/8 ; i++){
				for( b = 0 ; b < 8 ; b++ ){
					ir_flash( IR_FLASH_NEC_TIMES );
					if( data[i] & (0x01 << b) ){
						ir_wait( 3 * IR_FLASH_NEC_TIMES );
					}else{
						ir_wait( IR_FLASH_NEC_TIMES );
					}
				}
			}
			ir_flash( IR_FLASH_NEC_TIMES );		// send 'stop'
			break;
		case IR_SIRC:
			for(j=0;j<3;j++){
				t=5;						// 送信済シンボル基本単位(SYNCで送信)
				ir_flash( 4 * IR_FLASH_SIRC_TIMES );	// send 'SYNC_H'
				ir_wait(      IR_FLASH_SIRC_TIMES );	// send 'SYNC_L'
				for( b = 0 ; b < 7 ; b++ ){
					if( data[0] & (0x01 << b) ){
						ir_flash( 2 * IR_FLASH_SIRC_TIMES );
						t +=3 ;
					}else{
						ir_flash( IR_FLASH_SIRC_TIMES );
						t +=2 ;
					}
					ir_wait( IR_FLASH_SIRC_TIMES );
				}
		...
		case IR_AEHA:
		default:
			ir_flash( 8 * IR_FLASH_AEHA_TIMES );	// send 'SYNC_H'
			ir_wait(  4 * IR_FLASH_AEHA_TIMES);	// send 'SYNC_L'
			for( i = 0 ; i < data_len/8 ; i++){
				for( b = 0 ; b < 8 ; b++ ){
					ir_flash( IR_FLASH_AEHA_TIMES );
					if( data[i] & (0x01 << b) ){
						ir_wait( 3 * IR_FLASH_AEHA_TIMES );
					}else{
						ir_wait( IR_FLASH_AEHA_TIMES );
					}
				}
			}
			ir_flash( IR_FLASH_AEHA_TIMES );		// send 'stop'
			break;
	}
}
